...
def search_business_locations(business_type: str, area: str) -> str:
    ...
    maps_link = f"https://www.google.com/maps/search/{urllib.parse.quote(business_type + ' in ' + area)}"
    query = urllib.parse.quote(business_type + " " + area)
...
def get_area_info(area: str) -> str:
    ...
    url = "https://en.wikipedia.org/w/api.php?action=query&titles=" + query + "&prop=extracts&exintro=true&explaintext=true&format=json"
    maps_link = "https://www.google.com/maps/search/" + urllib.parse.quote(area)
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=10) as response:
            data = json.loads(response.read().decode())
        ...
        extract = "Could not fetch: " + str(e)
    return "=== " + area + " ===\nMaps: " + maps_link + "\n\n" + extract + "\n"

if __name__ == "__main__":
    print("MCP Server starting on port 8001...", flush=True)
